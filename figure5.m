...
rmpath(genpath('/Users/sheilawerth/Dropbox/Lab/Passive corner camera code review/'))
addpath(genpath('/Users/sheilawerth/Dropbox/Lab/Passive corner camera code review/ICCP2019/'))
...
%% LOAD DATA
datStr = 'syntheticDataFigure5.mat';
load(datStr)
...
penumbraIncident = sim.penumbra_light; % just incident light, floor albedo is uniform
penumbraIncident(removeRows) = [];
trueAmbientLight = sim.ambient_light;
...
% tuning parameters
% 1. hidden scene:
lam2 = 5e-4; % tuning parameter to promote smoothness of scene estimate

% 2. floor albedo:
lam5 = 10000; % sheila - gaussian prior in new prior
...
plotMeAmbient = plotMe(sim.ambient_light);
...
stepsize_f = 0.0158; % floor stepsize
...
if display && ~mod(outerIter,1)
    ...
    subplot(252)
    plot((1:imgDim)/imgDim*90,sim.scene/norm(sim.scene),'b.-');
    hold on;
