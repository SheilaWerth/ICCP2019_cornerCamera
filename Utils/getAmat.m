...
% The procedure:  
% upsample the sampling grid.  
% get the amat for that scene sampling.  
% then convolve and subsample the columns of the amat by the blur kernel to get the new
% amat.  
% For now:  only allow horizontal blur kernels.
...
% xx and zz are 1 by imgDim


pos = sceneIndexToPos(params, xx, zz, size(scene) );
% pos is 1 by US*imgDim by 3
...
% pp is a matrix where each row is a different coordinate [x;y;z]. Each column is a
% different scene element. 

ss = reshape(scene, size(scene,1)*size(scene,2), 3)';
% ss is full of ones, same size as pp

amat = zeros(params.imageH, params.imageV, size(pp,2));

% loop through columns of the scene
for i = 1:size(pp,2)
   tmp =  renderPt(params, ss(:,i), pp(:,i),i);
   amat(:,:,i) = tmp(:,:,1);
end
