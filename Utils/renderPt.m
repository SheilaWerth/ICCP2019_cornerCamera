function [ imOut] = renderPt(params,clr,pos,f)
...
% reorient pos and repeat accross each pixel, take difference between pos
% (in scene)
% and the xyz position of each pixel on the floor:
toLight = repmat(reshape(pos,[1,1,3]), [size(imx,1), size(imx,2), 1]) - toLight;

% now normalize toLight
denom = sum(toLight.^2, 3).^0.5; % find lenght of each three vector
toLight = toLight ./ repmat(denom, [1,1,3]); % scale each three vector by its length
% if f == 1
%     figure
%     subplot(131)
%     imagesc(imx)
...
imOut = renderBRDF(clr, toLight, params.unitVectorToObserver, ...
    params.imageNormal, params.brdf,f);
